...
for(g in gammas){
  plot(NA,NA,xlim=c(0.4,1), ylim=c(0,25),#up_ys[g]), 
       main=paste('Active-rank on top ',g, '%'),
       xlab='Activity quantile defining training dataset (%)',
       ylab='Total model score',xaxt='n')
...
plot(NA,NA,xlim=c(0.4,1), ylim=c(0,25), main='Mean Squared Error',
     xlab='Activity quantile defining training dataset (%)',ylab='Total model score',xaxt='n')
axis(1,at=c(0.4,.6,.8,1), labels = 100*c(0.4,.6,.8,1))
for (m in models){
  if(DO_DL | (!DO_DL & m != 'dl_l')){
    for(f in frac_fits){
...
legend('left',col=cols,legend = model_names,lwd=2,
      cex=.7,bty='n', lty=4)
